...
public class PropertyFilterResource {
	...
	@Secured
	@ApiOperation( 
		    value = "Retrieves the list of property filters", 
		    notes = "Return from the property list elements, theirs that allows to property filters"
		)
		@ApiResponses( {
		    @ApiResponse( code = 401, message = "If user is unauthorized" ),
	...
	public Response getPropertyFilters(@Context UriInfo uriInfo) {
		...
		List<HcFilter> filterList = propertyFilterService.loadFilterList(!filterNatureIdList.isEmpty(), 
				filterNatureIdList);
